      ...
      if (!boolFriend) {
        return false;
			...
			loadMoreBlock.classList.add('loading');
				...
				request.open('POST','updatefriends');
				request.addEventListener('readystatechange', function() {
 if ((request.readyState==4) && (request.status==200)) {
          if (boolSearch){
            loadMoreBlock.innerHTML = request.responseText;
          } else{
            if (request.responseText.trim() == "<div class='no-friend'><h3>У вас пока-что нет друзей :( </h3><a href='friend-search'>Найти друзей</a></div>") {
     ...
     function deleteFriend(id){
       ...
       var data = new FormData();
       data.append('id', id);
       request.open('POST','friend_deleted');
       request.addEventListener('readystatechange', function() {
         if ((request.readyState==4) && (request.status==200)) {
