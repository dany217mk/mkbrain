 ...
 let loadMoreBlock = document.getElementById('list');
      ...
      search.oninput = function(){
        ...
        scrollTop();
		  ...
		  function lazyScroll() {
        var header = document.getElementById('form-user');
      ...
      if (!boolUser) {
        return false;
			...
			loadMoreBlock.classList.add('loading');
				...
				request.open('POST','orgupdateusers');
				request.addEventListener('readystatechange', function() {
 if ((request.readyState==4) && (request.status==200)) {
          if (boolSearch){
            loadMoreBlock.innerHTML = request.responseText;
          } else{
            if (request.responseText.trim() == "<tr><h3>В данной организации пока-что нет пользователей</h3></tr>") {
