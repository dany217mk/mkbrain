const BORDER = 50;
      ...
      if (!boolFriend) {
        return false;
			...
			loadMoreBlock.classList.add('loading');
				...
				request.open('POST','updatemyrequests');
				request.addEventListener('readystatechange', function() {
				if ((request.readyState==4) && (request.status==200)) {
          if (boolSearch){
            loadMoreBlock.innerHTML = request.responseText;
          } else{
            if (request.responseText == "<div class='no-friend'><h3>Вы пока-что не отправляли заявок :( </h3><a href='friend-search'>Найти друзей</a></div>") {
     ...
     function deleteRequest(id){
       ...
       var data = new FormData();
       data.append('id', id);
       request.open('POST','request_deleted');
       request.addEventListener('readystatechange', function() {
         if ((request.readyState==4) && (request.status==200)) {
           ...
           if (request.responseText == "success") {
             ...
             if (loadMoreBlock.innerHTML.trim() == ""){
               loadMoreBlock.innerHTML = "<div class='no-friend'><h3>Вы пока-что не отправляли заявок :( </h3><a href='friend-search'>Найти друзей</a></div>";
               boolFriend = false;
             }
           } else{
