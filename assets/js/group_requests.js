const BORDER = 50;
      ...
      if (!boolFriend) {
        return false;
			...
			loadMoreBlock.classList.add('loading');
				...
				request.open('POST','updategrouprequests');
				request.addEventListener('readystatechange', function() {
				if ((request.readyState==4) && (request.status==200)) {
          if (boolSearch){
            loadMoreBlock.innerHTML = request.responseText;
          } else{
            if (request.responseText == "<div class='no-group'><h3>Вы пока-что не отправляли заявок :( </h3><a href='group-search'>Найти класс</a></div>") {
     ...
     function deleteRequest(id){
       ...
       var data = new FormData();
       data.append('id', id);
       request.open('POST','requestgroupdeleted');
       request.addEventListener('readystatechange', function() {
         if ((request.readyState==4) && (request.status==200)) {
           ...
           if (request.responseText == "success") {
             ...
             if (loadMoreBlock.innerHTML.trim() == ""){
               loadMoreBlock.innerHTML = "<div class='no-group'><h3>Вы пока-что не отправляли заявок :( </h3><a href='group-search'>Найти класс</a></div>";
               boolFriend = false;
             }
           } else{
